...
import { ApplicationTable, SkeletonApplicationTable } from "@/features/jobListingApplications/components/ApplicationTable";
import { getJobListingApplicationJobListingTag } from "@/features/jobListingApplications/db/cache/jobListingApplications";
import { deleteJobListing, toggleJobListingFeatured, toggleJobListingStatus } from "@/features/jobListings/actions/actions";
...
async function SuspendedPage({ params }: Props) {
    ...
    return <div className="space-y-6 max-w-6xl mx-auto p-4 @container">
        <div className="flex items-center justify-between gap-4 @max-4xl:flex-col @max-4xl:items-start">
            <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    <JobListingBadges jobListing={jobListing}></JobListingBadges>
                </div>
            </div>
            <div className="flex items-center gap-2 empty:-mt-4">
                <AsyncIf condition={() => hasOrgUserPermission("org:job_listing_applications:update")}>
                ...
                <AsyncIf condition={() => hasOrgUserPermission("org:job_listing_applications:delete")}>
                    <ActionButton variant="destructive" action={deleteJobListing.bind(null, jobListing.id)}>
        ...
        <div className="space-y-6">
            <h2 className="text-xl fonr-semibold">Applications</h2>
...
function StatusUpdateButton({ status, id }: { status: JobListingStatus, id: string }) {
    const button = <ActionButton
        ...
        requireAreYouSure={getNextJobListingStatus(status) === "published"} areYouSureDescription="This will immediately show this job listing to all users"
        variant="outline"
    >
    ...
    return (
        <AsyncIf condition={() => hasOrgUserPermission("org:job_listing_applications:change_status")}>
