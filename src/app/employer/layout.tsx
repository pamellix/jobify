    ...
    return <AppSidebar content={
        <>
            <SidebarGroup>
                ...
                <AsyncIf condition={() => hasOrgUserPermission("org:job_listing_applications:create")}>
            ...
            <SidebarNavMenuGroup className="mt-auto" items={[
                { href: "/", icon: <ClipboardListIcon />, label: "Job Board", authStatus: "signedIn" },
            ]} />
...
async function JobListingMenu({ orgId }: { orgId: string }) {
    ...
    if (jobListings.length === 0 && (await hasOrgUserPermission("org:job_listing_applications:create"))) {
        return (
            <SidebarMenu>
                <SidebarMenuItem>
