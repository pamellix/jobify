...
export const rankApplication = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const [coverLetter, resume, jobListing] = await Promise.all([getCoverLetter, getResume, getJobListing])

        if (resume == null || jobListing == null) return;

        await applicantRankingAgent.run(JSON.stringify({ coverLetter, resume, jobListingId, userId }))
    }
)