...
import { formatExperienceLevel, formatJobListingLocation, formatJobType, formatLocationRequirement } from "@/features/jobListings/lib/formatters";
import { formatWage } from "@/features/jobListings/components/jobListingBadges";
    ...
    return <Tailwind config={tailwindConfig}>
        <Html>
            ...
            <Container className="font-sans">
                ...
                <Text>Hi {userName}. Here are the new job listings that were posted today:</Text>
                <Section>
                    {jobListings.map(jobListing => (
                        <div key={jobListing.id} className="bg-card text-card-foreground rounded-lg border p-4 border-primary border-solid mb-6">
                            <Text className="leading-none fonr-semibold text-xl my-0">
                                {jobListing.title}
                            </Text>
                            ...
                            <div className="mb-5">
                                {getBadges(jobListing).map((badge, index) => (
                                    <div key={index} className="inline-block rounded-md border-solid border font-medium w-fit text-foreground text-sm px-3 py-1 mb-1 mr-1">
                                        {badge}
                                    </div>
                                ))}
                            </div>
